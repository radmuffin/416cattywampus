...
if __name__ == '__main__':
    NVERTS, NEDGES, SRC, TARG = map(int, input().split())


    '''
    NSTUDENTS = 8
    NICE      = 6
    G = PushRelabel(NSTUDENTS+NICE+2) # +2 for adding src/dest to what the problem included
    '''

    G = PushRelabel(NVERTS)

    for _ in range(NEDGES):
        U,V,C = map(int, input().split())
        G.addEdge(U,V,C)

    flow = G.calc(SRC,TARG)
    #pr(flow)

    edges = G.retrieveFlow()
    #print(edges)
    print(NVERTS,flow,len(edges))
    for e in edges:
